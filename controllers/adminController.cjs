...
const Blog = require('../models/Blog.cjs');  // ✅ ADD THIS
...
exports.adminLogin = async (req, res) => {
    try {
        ...
        console.log('🔑 Admin Login Attempt:', { username, email, hasPassword: !!password });
        ...
        console.log('🔍 Looking for admin with filter:', filter);
        const admin = await Admin.findOne(filter);

        if (!admin) {
            console.log('❌ Admin not found with filter:', filter);
        ...
        const passwordMatch = await admin.correctPassword(password, admin.password);
        if (!passwordMatch) {
...
exports.createBlog = async (req, res) => {
    try {
        const { title, excerpt, content, featuredImage, categories, tags, status, isFeatured, metaTitle, metaDescription, language } = req.body;
        ...
        const blogData = {
            ...
            language: language || 'EN',
            author: req.admin._id // Use _id instead of id
        };
        ...
        if (err.name === 'ValidationError') {
            const errMessages = Object.values(err.errors).map(e => e.message);
            return res.status(400).json({
                success: false,
                error: errMessages.join(', ')
            });
        }
